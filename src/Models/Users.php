...
class Users extends Models
{
	...
	{
		$pstmt = $this->db->prepare('SELECT * FROM users');
		$pstmt->execute();
	...
	{
		$pstmt = $this->db->prepare('SELECT * FROM users WHERE id = :id');
	...
	{
		$pstmt = $this->db->prepare('SELECT * FROM posts WHERE userId = :userId');
	...
	public function all()
	{
		...
		$pstmt = $this->db->prepare('SELECT * FROM users ORDER BY id LIMIT :start, :length');
	...
	public function allPosts($id)
	{
		...
		$pstmt = $this->db->prepare('SELECT * FROM posts WHERE userId = :userId ORDER BY id LIMIT :start, :length');
		$pstmt->bindValue(':userId', $id, \PDO::PARAM_INT);
		$pstmt->bindValue(':start', $_POST['start'], \PDO::PARAM_INT);
		$pstmt->bindValue(':length', $_POST['length'], \PDO::PARAM_INT);
		$pstmt->execute();
